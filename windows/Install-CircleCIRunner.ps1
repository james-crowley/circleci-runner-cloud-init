...
$keeperTask = Register-ScheduledTask -Force -TaskName "CircleCI Runner Agent session keeper" -User $username -Password $passwd -Action (New-ScheduledTaskAction -Execute powershell.exe -Argument "-Command `"cmdkey.exe /add:TERMSRV/localhost /user:$username '/pass:$passwd' ; reg.exe ADD 'HKCU\Software\Microsoft\Terminal Server Client' /v AuthenticationLevelOverride /t REG_DWORD /d 0x0 /f ; reg.exe ADD 'HKCU\Software\Microsoft\ServerManager' /v CheckedUnattendLaunchSetting /t REG_DWORD /d 0x0 /f ; mstsc.exe /v:localhost; while (`$true) { Write-Host 'Sleeping'; Start-Sleep 30; }`"") -Settings $commonTaskSettings -Trigger (New-ScheduledTaskTrigger -AtStartup)

# Preparing config template
Write-Host "Preparing a config template for CircleCI Runner Agent"
@"
api:
  auth_token: "<<AUTH_TOKEN>>" # FIXME: Specify your runner token
  # On server, set url to the hostname of your server installation. For example,
  # url: https://circleci.example.com
runner:
  name: "<<RUNNER_NAME>>" # FIXME: Specify the name of this runner instance
  mode: single-task
